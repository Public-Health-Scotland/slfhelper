...
slf_extract <- read_slf_episode(c( "1819", "1920"),
                                ...
                                # Select columns
                                col_select = c("year", "anon_chi", "recid", 
...
el_1819 <- read_slf_individual(year = "1819", 
...
hb_lookup <- 
  ...
  select(hb2019 = HealthBoardArea2019Code,
         hb_desc = HealthBoardArea2019Name)

# read in data required from slf individual file - filter for 2018/19
indiv_1819 <- read_slf_individual(year = "1819",
                                  # Select columns of interest
                                  col_select = c("hb2019", "cij_el", "cij_non_el",
...
indiv_1819_inc_totals <- indiv_1819 %>%
  ...
  mutate(elective_beddays = acute_el_inpatient_beddays + mh_el_inpatient_beddays +
                            gls_el_inpatient_beddays,
...
sa_1920 <- read_slf_episode(year = "1920",
                            # select columns
...
ab_1819 <- read_slf_individual(year = "1819",
                               # select columns 
                               col_select = c("acute_cost", "gls_cost", "mh_cost", "mat_cost",
...
deaths <- read_slf_episode(year = c("1516", "1617", "1718", "1819", "1920"),
                           # select columns
...
dementia_deaths <- deaths %>%
  ...
  mutate(diag_3d = str_sub(deathdiag1, 1, 3),
...
hscp_lookup <- read_csv("/conf/linkage/output/lookups/Unicode/Geography/Scottish Postcode Directory/Codes and Names/Integration Authority 2019 Lookup.csv") %>%
  select(hscp2019 = IntegrationAuthority2019Code,
...
output_table_7 <- ms_1819 %>%
  ...
  summarise(pis_paid_items = sum(pis_paid_items),
            pis_cost = sum(pis_cost),
            patients = sum(ms)) %>%
  ungroup() %>%
  # calculate number of items / cost per patient
  mutate(items_per_patient = pis_paid_items/patients,
...
gc_deaths <- read_slf_episode(year = "1920",
                              # select columns
...
ae <- read_slf_episode(year = c("1819", "1920"),
                       # select columns
...
smr_1920 <- read_slf_episode(year = "1920",
                              col_select = c("anon_chi", "record_keydate1", "record_keydate2",
...
output_table_9 <- pat_1920 %>%
  ...
  group_by(hscp2019, hscp_desc) %>%
  # sum up measures
  summarise(admissions=sum(ger_med),
